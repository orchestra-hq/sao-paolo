...
- Survive a run-results entry with an empty `timing` array, which dbt 2.x can write. `get_last_updated_from_run_results` indexed it directly, threw `IndexError`, and the node was then skipped entirely by `update_state` — so it kept no checksum and no source snapshot, went dirty on every subsequent run, and the run's source freshness was never recorded against it. The artifact's `metadata.generated_at` is now used as the fallback. A `status` of `Success` is also accepted alongside `success`.

...
- Node selection is resolved through the manifest, because 2.x's programmatic `dbt ls` result returns fully-qualified names rather than the file paths `--output path` puts on stdout.

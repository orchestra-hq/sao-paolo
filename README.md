...
| `scope_source_freshness_to_selection` | bool | `false` | When `true`, only sources upstream of the current `--select`/`--exclude`/`--selector` are actually checked for freshness -- the `dbt source freshness` invocation itself is unchanged, but the selection already resolved for node reuse (via `dbt ls`) is walked through dbt's own dependency graph (`target/manifest.json`) to find which sources feed into it, and freshness for everything else is skipped. When `false` (default), freshness is checked for every source in the project regardless of selection, matching prior behaviour. |

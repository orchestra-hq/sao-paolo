...
**Views:** that metadata timestamp reflects when the relation's *definition* last changed, not its data — for a view, it's effectively frozen unless someone redefines it, so an unchanged timestamp does **not** mean unchanged data. Orchestra checks the relation kind (via dbt's own generic `adapter.get_relation()` — no per-warehouse SQL) before trusting that timestamp, and for a view treats the source as always having new data rather than comparing a frozen value. Only sources relying on implicit metadata freshness are checked; one with `loaded_at_field`/`loaded_at_query` configured is skipped, since that reads real data instead. If the relation kind can't be determined, the source keeps dbt's normal metadata-freshness behavior rather than being forced to rebuild.

To opt out of implicit freshness entirely — for views and ordinary tables alike — set `require_explicit_source_freshness = true` (or `ORCHESTRA_REQUIRE_EXPLICIT_SOURCE_FRESHNESS=true`). Sources without `loaded_at_field`/`loaded_at_query` are then excluded from state-aware orchestration and models depending on them always run; sources with an explicit config keep working as normal.

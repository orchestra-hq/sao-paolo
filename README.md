...
For adapters without a registered fallback, if both `loaded_at` settings are missing, Orchestra follows dbt's `FreshnessRunner` behavior (which may surface as warnings or a non-actionable result depending on dbt and the warehouse). On adapters that support metadata-based freshness (e.g. Snowflake, BigQuery, Redshift), this means `max_loaded_at` comes from the relation's warehouse metadata (e.g. Snowflake's `INFORMATION_SCHEMA.TABLES.LAST_ALTERED`).

**Views:** that metadata timestamp reflects when the relation's *definition* last changed, not its data — for a view, it's effectively frozen unless someone redefines it, so an unchanged timestamp does **not** mean unchanged data. To avoid reusing downstream models forever off a frozen timestamp, Orchestra records each such source's warehouse relation type (`VIEW`, `BASE TABLE`, …) and always treats view-backed sources as having new data rather than comparing timestamps.

The lookup uses dbt's own generic `adapter.get_relation()`, so it is adapter-agnostic rather than per-warehouse SQL, and it happens during the freshness run on the connection that is already open. It goes through dbt's relation cache (at most one lightweight "list relations in this schema" query per schema), is cached across runs in state under `source_relation_types`, and only covers sources relying on implicit metadata freshness. Sources with `loaded_at_field`/`loaded_at_query` configured are skipped entirely, since those read real data rather than relation metadata. If the kind can't be determined the source keeps its previous behaviour rather than being forced to rebuild.
